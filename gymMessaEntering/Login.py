...
studentList.column('c2', width=60, anchor='center')
studentList.column('c3', width=40, anchor='center')
studentList.column('c4', width=40, anchor='center')
studentList.column('c5', width=70, anchor='center')
...
studentList.heading('c5', text='私人教练')
...
# Treeview组件
courseList = tkinter.ttk.Treeview(frame, columns=('c1', 'c2', 'c3'),
                                       show="headings")
courseList.column('c1', width=70, anchor='center')
courseList.column('c2', width=70, anchor='center')
courseList.column('c3', width=70, anchor='center')


courseList.heading('c1', text='课程号')
courseList.heading('c2', text='课程名')
courseList.heading('c3', text='教练编号')
...
# 在窗口上放置用来显示通信录信息的表格，使用Treeview组件实现
frame = tkinter.Frame(root)
frame.place(x=580, y=180, width=230, height=280)
# Treeview组件
scList = tkinter.ttk.Treeview(frame, columns=('c1', 'c2', 'c3'),
                                       show="headings")
scList.column('c1', width=80, anchor='center')
scList.column('c2', width=50, anchor='center')
...
def add():
    ...
    c.execute("INSERT INTO students (Sno, Sname,Ssex,Sage,Scoach) VALUES ('%s', '%s','%s','%s','%s')" % (sno, sname,ssex,sage,sdept))
    db.commit()
...
def update():
    ...
    db = pymysql.connect(host="localhost", user="root",
                         password="root", db="stu", port=3306, charset='utf8')
    c = db.cursor()
    SQL="UPDATE students SET Scoach='%s' WHERE Sno='%s'"
...
def ageadd():
    ...
    c.execute("call procUpdate2(16001)")
